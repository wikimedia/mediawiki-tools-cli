...
func reGenerateGerritCommands(spec Spec) {
	deleteGeneratedFilesFromDirectory("commands/gerrit")
	for _, c := range spec {
		f := jen.NewFile("gerrit")
		f.Comment("Code generated by tools/code-gen/main.go. DO NOT EDIT.")

		f.Add(cobraCommandFuncWithSubCommands(c, "")...)

		filePath := "commands/gerrit/" + codegen.ForFileName(c.Use) + ".gen.go"
		err := f.Save(filePath)
		if err != nil {
			logrus.Fatal(err)
		}
...
func cobraCommandBlock(namePartForFunc string, c Command) jen.Code {
	...
	for _, flag := range c.StringFlags {
		...
		if flag.Required {
			addFlagsToCmd = append(addFlagsToCmd, jen.Id("_").Op("=").Id("cmd").Dot("MarkFlagRequired").Call(jen.Lit(flag.Name)).Line())
		}
